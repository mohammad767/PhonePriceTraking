...
def technolife_scraper_by_brand(brand,parametrs={},series="") :
    base_url = "https://www.technolife.com"
    
    if series :
        url = f"https://www.technolife.com/category/mobile/mobile-phone/brand-{brand}/{series}"
        
    url = f"https://www.technolife.com/category/mobile/mobile-phone/brand-{brand}"
    headers = {
            "User-Agent" : "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3"
        }
    try :
        response = requests.get(url=url,headers=headers,params=parametrs,timeout=20)
    except requests.Timeout:
                print("Error: The request timed out while trying to retrieve the webpage.")
                return []
...
def choose_brand():
    brands = ["xiaomi", "nokia", "samsung", "honor", "apple"]
    while True:
...
def choose_extra_filters():
    ...
    min_price = input("Enter The Min Price (blank to skip): ")
    if min_price.strip():
        params["pfrom"] = int(min_price)

    ram_map = {"8": "195", "6": "194", "4": "193"}
    add_ram = input("Do You Want To Filter RAM? (Y/N): ").lower()
    if add_ram == "y":
        while True:
            ram = input("Enter RAM Amount (8/6/4): ")
            if ram in ram_map:
...
def choose_series(brand):
    ...
    if brand not in series_map:
        print("I Didn't Code Series For This Brand")
        return ""

    add_series = input("Do You Want To Choose A Series? (Y/N): ").lower()
    if add_series != "y":
        return ""
    ...
    while True:
        ...
        pick = input("Enter Your Choice: ").lower()
        if pick in options:
            return options[pick]
        print("Invalid Serie")
...
def handle_scrape_by_brand():
    ...
    products = technolife_scraper_by_brand(brand, params, series)
    print(products)
    return products
...
def digikala_scraper(category="mobile-phone", parametrs={}, page=1):
    """
    Scrapes product listings from Digikala's category search API.
    category: e.g. "mobile-phone"
    parametrs: extra query params (e.g. {"brand[]": "apple", "price[min]": 5000000})
    ...
    if response.status_code == 200:
        ...
        for phone in products_data:
            ...
            if not name:
                print("Error: Could not find the name for a phone.")
                continue
            ...
            if price is None:
                print("Error: Could not find the price for a phone.")
                continue
            ...
            product_list.append(Product(name, price, product_url))
...
def handle_digikala_scrape():
    ...
    page = 1
    while True:
        products = digikala_scraper(category="mobile-phone", parametrs=params, page=page)
        if not products:
            break
        all_products.extend(products)

        more_pages = input(f"Got {len(products)} products on page {page}. Fetch next page? (Y/N): ").lower()
        if more_pages != "y":
            break
        page += 1
